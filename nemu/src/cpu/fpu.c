...
//	printf("C\n");
	...
	if(!overflow) {
		...
		uint32_t grs = sig_grs & 0x00000007;
		if(grs > 0x00000004){
			sig_grs >>= 3;
			sig_grs += 1;
		}
		else if(grs <0x00000004){
	//			printf("D,sig=%x\n",(uint32_t)sig_grs);
				sig_grs >>= 3;
		}
		else{
			if((sig_grs&0x00000008) == 0){
					sig_grs >>= 3;
			}
			else{
				sig_grs >>= 3;
		...
		else if(((sig_grs >> 23) ==1)&&(exp == 0))	exp++;
...
uint32_t internal_float_add(uint32_t b, uint32_t a) {
	...
	if(fa.exponent > fb.exponent) {
		fa.val = b;				//fa always is the less one(for exp)
		fb.val = a;
	}
...
uint32_t internal_float_mul(uint32_t b, uint32_t a) {
	...
	if((exp_zero_a == 1)&&(exp_zero_b == 1))
	{
			exp_res = 1;
			sig_res = 0;
	}
	else if((exp_zero_a ==1)&&(exp_zero_b != 1))
		exp_res = fb.exponent-149;
	else if((exp_zero_a != 1)&&(exp_zero_b ==1 ))
		exp_res = fa.exponent-149;
	else{ 
		if((fa.exponent+fb.exponent- 150) > 0x000000ff)
			exp_res = 0xff;
		else 
